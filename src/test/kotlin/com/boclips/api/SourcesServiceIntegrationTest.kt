...
class SourcesServiceIntegrationTest : AbstractIntegrationTest() {
    ...
    fun getAllSources_returnsAllSources() {
        val sources = sourcesService.getAllSources().collectList().block()
    ...
    fun createSource_whenDoesNotExist_createsNewSource() {
        val created = sourcesService.createSource("TeD").block()

        assertThat(created).isTrue()
        val source = sourcesService.getAllSources().collectList().block()!!.find { it.name == "TeD" }!!
    ...
    fun createSource_whenExists_doesNotCreateANewOne() {
        val created = sourcesService.createSource("Sky News").block()

        assertThat(created).isFalse()
        assertThat(sourcesService.getAllSources().collectList().block()).hasSize(1)
    }
}