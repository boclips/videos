...
class CustomMetadataFileValidator(val videoRepository: VideoRepository) {
    ...
    private fun validateMappings(items: List<RawCustomMappingMetadata>): CustomMetadataValidationResult {
        findMissingColumns(items.firstOrNull())?.let { missingColumn -> return missingColumn }
    ...
    private fun findMissingColumns(header: RawCustomMappingMetadata?): MissingColumn? {
        if (header == null) {
            return null
        }
