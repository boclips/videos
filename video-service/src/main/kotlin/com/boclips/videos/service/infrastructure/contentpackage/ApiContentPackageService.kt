...
class ApiContentPackageService(
    ...
    override fun getAccessRules(id: ContentPackageId, privateChannels: Set<ChannelId>): AccessRules? =
        try {
            val resource = contentPackagesClient.find(id.value).accessRules
            AccessRules(
                videoAccess = accessRulesConverter.toVideoAccess(accessRules = resource, privateChannels = privateChannels),
