...
class VideoAnalysisService(
    ...
    fun videoAnalysed(videoAnalysed: VideoAnalysed) {
        ...
        SubjectClassificationService.logger.info { "Updating analysed video $videoId" }
        val video = try {
            videoRepository.find(VideoId(videoId))
        } catch (e: Exception) {
            SubjectClassificationService.logger.error(e) { "Error looking up video $videoId" }
            return
        }

        if (video == null) {
            SubjectClassificationService.logger.info { "Could not find video $videoId" }
            return
        }

        SubjectClassificationService.logger.info { "Updating metadata of video $videoId" }
        try {
            updateMetadata(video, videoAnalysed)

            SubjectClassificationService.logger.info { "Metadata of video $videoId updated" }
        } catch (e: Exception) {
            SubjectClassificationService.logger.error(e) { "Error updating metadata of video $videoId" }
        }

        if (videoAnalysed.transcript.isBlank()) {
            SubjectClassificationService.logger.info { "Deleting any existing auto-generated captions of video $videoId" }
            try {
                playbackRepository.deleteAutoGeneratedCaptions(video.playback.id, videoAnalysed.language)
                SubjectClassificationService.logger.info { "Not uploading captions of video $videoId because transcript is blank" }
            } catch (e: Exception) {
                SubjectClassificationService.logger.error(e) { "Error trying to delete existing captions of video $videoId" }
            }

            return
        }

        SubjectClassificationService.logger.info { "Uploading captions of video $videoId" }
        try {
            uploadCaptions(video, videoAnalysed)
            SubjectClassificationService.logger.info { "Captions of video $videoId uploaded" }
        } catch (e: Exception) {
            SubjectClassificationService.logger.error(e) { "Error uploading captions of video $videoId" }
        }
    }
