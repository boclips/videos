...
class MongoChannelRepositoryIntegrationTest : AbstractSpringIntegrationTest() {
    ...
    fun `find by visibility`() {
        mongoChannelRepository.create(createChannel(name = "coke", visibility = ChannelVisibility.HIDDEN))
        mongoChannelRepository.create(createChannel(name = "pepsi", visibility = ChannelVisibility.VISIBLE))

        val results = mongoChannelRepository.findAll(listOf(ChannelFilter.HiddenFilter(true)))
