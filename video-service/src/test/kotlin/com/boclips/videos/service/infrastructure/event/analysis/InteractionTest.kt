...
class InteractionTest {
    ...
    fun `fromPlaybackEvents combines events with the same player and video id`() {
        ...
        val event2 = TestFactories.createPlaybackEvent(
            playerId = "player-id",
            videoId = "123",
            captureTime = now.minusMinutes(1)
        )

        val interactions = Interaction.fromPlaybackEvents(listOf(event1, event2))

        assertThat(interactions).hasSize(1)
        assertThat(interactions[0]).isEqualTo(
            createInteraction(
                timestamp = now,
                description = "Watch 1m 0s of 123 (duration 1m 0s).",
                related = emptyList()
            )
        )
    }
    ...
    fun `fromSearchAndPlaybackEvents nests playback interactions`() {
        ...
        val interactions =
            Interaction.fromSearchAndPlaybackEvents(listOf(SearchAndPlayback(searchEvent, listOf(playbackEvent))))
    ...
    fun `sortRecursively orders at root level`() {
        val firstInteraction =
            createInteraction(timestamp = now.minusMinutes(2), description = "first", related = emptyList())
        val secondInteraction =
            createInteraction(timestamp = now.minusMinutes(1), description = "second", related = emptyList())
    ...
    fun `sortRecursively orders nested interactions`() {
        ...
        val sorted =
            sortRecursively(listOf(createInteraction(timestamp = now, description = "root", related = related)))
    ...
    fun `sortRecursively uses nested event max timestamp for sorting root level`() {
        ...
        val rootInteraction =
            createInteraction(timestamp = now.minusMinutes(2), description = "root", related = listOf(related))
