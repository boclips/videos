...
class InteractionsFormatterTest {
    ...
    fun format() {
        ...
        val interactions = listOf(
            createInteraction(
                timestamp = time, description = "root", user = User(false, "teacher-internal-id"), related = listOf(
                    createInteraction(timestamp = time.plusHours(1), description = "nested", related = emptyList())
                )
            ),
            createInteraction(
                timestamp = time.plusHours(2),
                description = "another",
                user = User(true, "employee-internal-id"),
                related = emptyList()
            )
        )
        ...
        assertThat(formatted).isEqualTo(
            """>  2018-02-03T12:11:12Z another by Boclips employee ID: employee-internal-id
                  |>  2018-02-03T10:11:12Z root by teacher ID: teacher-internal-id
