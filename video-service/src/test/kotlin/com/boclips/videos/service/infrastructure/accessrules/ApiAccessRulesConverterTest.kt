...
class ApiAccessRulesConverterTest : AbstractSpringIntegrationTest() {
    ...
    @Nested
    inner class ToVideoAccess {
        @Test
        fun `converts empty list to everything video access`() {
            val accessRules = apiAccessRulesConverter.toVideoAccess(emptyList(), emptySet())
            assertThat(accessRules).isEqualTo(VideoAccess.Everything(emptySet()))
        }
    ...
    @Nested
    inner class ToCollectionAccess {
        @Test
