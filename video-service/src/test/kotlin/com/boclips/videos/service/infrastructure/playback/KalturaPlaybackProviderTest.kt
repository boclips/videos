...
class KalturaPlaybackProviderTest : AbstractSpringIntegrationTest() {
    ...
    fun `uploads captions if there are no captions in Kaltura already`() {
        val existingArabicCaptions = createKalturaCaptionAsset(language = KalturaLanguage.ARABIC)
    ...
    fun `does not upload captions if there are manually-created captions in Kaltura already`() {
        val existingCaptions = createKalturaCaptionAsset(label = "English", language = KalturaLanguage.ENGLISH)
        fakeKalturaClient.addMediaEntry(createMediaEntry(referenceId = "ref-id"))
    ...
    fun `replaces captions if there are only automatically-created captions in Kaltura`() {
        val existingCaptions = createKalturaCaptionAsset(label = "English (auto-generated)", language = KalturaLanguage.ENGLISH)
        fakeKalturaClient.addMediaEntry(createMediaEntry(referenceId = "ref-id"))
        fakeKalturaClient.createCaptionsFile("ref-id", existingCaptions, "old captions")
        val playbackId = PlaybackId(type = PlaybackProviderType.KALTURA, value = "ref-id")

        val newCaptions = createCaptions(language = "en-UK", autoGenerated = true, content = "new captions")
        kalturaPlaybackProvider.uploadCaptions(playbackId, newCaptions)
