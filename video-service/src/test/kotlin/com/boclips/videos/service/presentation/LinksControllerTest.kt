        ...
        mockMvc.perform(get("/v1").asTeacher(userId))
            ...
            .andExpect(
                jsonPath(
                    ...
                    containsString("{?query,sort_by,include_tag,exclude_tag,duration_min,duration_max,released_date_from,released_date_to,source,age_range_min,age_range_max,size,page,subject,subjects_set_manually,promoted,content_partner,type,is_classroom}")
                )
            )
            ...
            .andExpect(
                jsonPath(
                    ...
                    endsWith("collections?public=true{&query,subject,projection,page,size}")
                )
            )
            .andExpect(
                jsonPath(
                    "$._links.searchCollections.href",
                    endsWith("collections{?query,subject,public,projection,page,size}")
                )
            )
            .andExpect(
                jsonPath(
