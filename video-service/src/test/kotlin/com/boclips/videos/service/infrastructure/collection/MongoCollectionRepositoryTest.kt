...
class MongoCollectionRepositoryTest : AbstractSpringIntegrationTest() {
    ...
    inner class RetrieveCollections {
        ...
        fun `can retrieve public collections`() {
            val publicCollection = collectionRepository.create(
                owner = UserId(value = "user1"),
                title = "Starting Title",
                createdByBoclips = false
            )
            ...
            collectionRepository.update(publicCollection.id, CollectionUpdateCommand.ChangeVisibilityCommand(true))
            collectionRepository.update(publicCollection2.id, CollectionUpdateCommand.ChangeVisibilityCommand(true))

            val publicCollections = collectionRepository.getPublic(PageRequest(0, 10))
        ...
        fun `can retrieve pages of public collections ordered by last edited`() {
            val publicCollection1 = collectionRepository.create(
                owner = UserId(value = "user1"),
                title = "Starting Title",
                createdByBoclips = false
            )
            ...
            collectionRepository.update(publicCollection1.id, CollectionUpdateCommand.ChangeVisibilityCommand(true))

            Thread.sleep(500)

            collectionRepository.update(publicCollection2.id, CollectionUpdateCommand.ChangeVisibilityCommand(true))

            val firstPage = collectionRepository.getPublic(PageRequest(0, 1))
            ...
            val lastPage = collectionRepository.getPublic(PageRequest(1, 1))
