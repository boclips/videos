...
class VideoControllerIntegrationTest : AbstractSpringIntegrationTest() {
    ...
    inner class GetCsv {
        @Test
        fun `gets a csv with metadata based on provided ids and adds caption url and extension`() {
            val playbackId = PlaybackId.from("playback-id", PlaybackProviderType.KALTURA.toString())
            val playbackId2 = PlaybackId.from("playback-id2", PlaybackProviderType.KALTURA.toString())
            val video1 = saveVideo(title = "test name 1", playbackId = playbackId)
            val video2 = saveVideo(title = "test name 2", playbackId = playbackId2)
            ...
            val captionsInOtherFormat = TestFactories.createCaptions(
                language = Locale.UK,
                content = "bla bla bla",
            ...
            kalturaPlaybackProvider.uploadCaptions(playbackId2, captionsInOtherFormat)
